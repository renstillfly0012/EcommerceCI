...
class Item extends CI_Controller
{
    ...
    public function edit($id)
    {
        ...
        $this->form_validation->set_rules('description', 'Item Description', 'required');
        $this->form_validation->set_rules('price', 'Item Price', 'required|numeric|greater_than[0]');
        $this->form_validation->set_rules('category', 'Item Category');
        $this->form_validation->set_rules('int_qty', 'Item Quantity', 'required|greater_than[0]');

        if ($this->form_validation->run() == FALSE) {
            $this->viewedit($id);
        } else {
            $this->load->model('Item_model');
            $this->Item_model->update($id, $name, $description, $price,$category, $int_qty);
            redirect('item/index');
        }
    }
